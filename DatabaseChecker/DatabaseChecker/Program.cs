    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                //conn.ConnectionString = "Server=[Glass];Database=[Jenzabar];Trusted_Connection=true";
                conn.ConnectionString = "Data Source = Glass; Initial Catalog = Jenzabar; Integrated Security = True; Connect Timeout = 30; Encrypt = False; TrustServerCertificate = False; ApplicationIntent = ReadWrite; MultiSubnetFailover = False";
                conn.Open();
                // Create the command
                SqlCommand command = new SqlCommand("SELECT * FROM dbo.AdmMap WHERE ComplCode = 1", conn);
                // Add the parameters.
